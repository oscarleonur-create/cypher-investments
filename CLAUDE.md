...
| News retrieval | pulled to explain something a free detector already found. Keywords, never sentences — measured, a full-sentence query scored 0.393 against 0.924 for keywords. **One scheduled pull** (user decision, 2026-09-24): the review job searches each held position's confirmed angles once a day, largest position first, within a hard budget of 20 Tavily queries — Grok 4.7 shipped and was never seen because nothing asked about it. **A second scheduled pull, for exits** (user decision, 2026-09-26): a distress sweep per held name at 08:15 and 12:30, weekends included (`bankruptcy default delisting`, `fraud investigation restructuring`); the model reads it and an exit-grade report from ≥2 independent outlets is an EXIT labeled unconfirmed, one outlet a REVIEW (`entry.distress`) |
